...
#PLOT NMDS
###############################################################
plotNMDS<-function(nmds.result = NULL, data = NULL, predictor = NULL, title = "", labels = NULL){
  
  library(ggplot2)
  library(ggrepel)
  library(viridis)
  
  #GENERATE ORDIGRID OBJECT FROM NMDS FOR PLOTTING
  ####################################################
  # https://oliviarata.wordpress.com/2014/07/17/ordinations-in-ggplot2-v2-ordisurf/
  generateOrdigrid<-function(nmds.result, data, predictor){
    
    #created the ordisurf object
    ordi <- ordisurf(nmds.result ~ data[, predictor])
    
    #extracts the ordisurf object
    ordi.grid <- ordi$grid
    
    #get x and ys
    ordi.grid.2 <- expand.grid(x = ordi.grid$x, y = ordi.grid$y)
    
    #unravel the matrix for the z scores
    ordi.grid.2$z <- as.vector(ordi.grid$z)
    
    #gets rid of the nas
    ordi.grid.ready <- data.frame(na.omit(ordi.grid.2))
    
    return(ordi.grid.ready)
  }
  
  #computing ordisurf object/s of the predictor/s
  ordi.object<-generateOrdigrid(nmds.result=nmds.result, data=data, predictor=predictor)
  
  #this puts the NMDS scores for the plots into a new dataframe
  data$NMDS1 <- nmds.result$points[ ,1]
  data$NMDS2 <- nmds.result$points[ ,2]
  
  #plot
  nmds.plot<-ggplot(data, aes(x=NMDS1, y=NMDS2)) +
    # geom_raster(data = ordi.object, aes(x=x, y=y, fill=z)) +
    stat_contour(data = ordi.object, aes(x=x, y=y, z=z, colour = ..level..), binwidth = (max(ordi.object$z)-min(ordi.object$z))/10, size=1) +
    geom_point(size = 2) +
    geom_label_repel(aes(NMDS1, NMDS2, label = labels), box.padding = 0.25, point.padding = 0.5, segment.color = 'grey50', color='grey50') +
    theme_bw() +
...
simulatePopulation = function(parameters=NULL,
  ...
  for(i in selected.species){
    ...
    if(drivers.input=="dataframe"){
      ...
      if(length(driver.A)>0 & length(driver.B)>0 & (sum(is.na(driver.B))==0)){
...
toRegularTime <- function(x, time.column, interpolation.interval, columns.to.interpolate=c("Suitability", "Driver.A", "Pollen")){
  ...
  x.columns <- 1:dim(x)[2] #from 2 to avoid changing the original sim
...
extractMemoryFeatures <- function(analysis.output, exogenous.component, endogenous.component, sampling.subset=NULL, scale.strength = TRUE){
...
plotExperiment = function(experiment.output, parameters.file, experiment.title, sampling.names, legend.position, R2=NULL, filename=NULL, strip.text.size=12, axis.x.text.size=8, axis.y.text.size=12, axis.x.title.size=14, axis.y.title.size=14, title.size=18, caption=""){
  ...
  experiment.plot <- ggplot(data=simulation.df, aes(x=Lag, y=median, group=Variable, color=Variable, fill=Variable)) +
    ...
    facet_wrap("name", ncol = length(unique(simulation.df$sampling))) + 
    xlab("Lag (years)") + 
    ylab("Relative importance") +
    geom_hline(yintercept=seq(0, 40, by=20), color="gray", alpha=0.3, size=0.2) +
    geom_vline(xintercept=seq(0, 240, by=20), color="gray", alpha=0.3, size=0.2) +
    theme(strip.text.x = element_text(size = strip.text.size), 
          ...
          plot.caption = element_text(hjust = 0, size = axis.y.title.size)) +
